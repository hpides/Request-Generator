...
class Request : Atom() {
     ...
     */
    var requestParams: Array<String?>? = null
    /**
    ...
    var implicitNotFailedAssertion: RequestIsSent? = null
    ...
    override fun perform() {
        ...
        if (implicitNotFailedAssertion == null) {
            ...
            implicitNotFailedAssertion!!.setName("Request \"$name\" is sent")
        }
        when (verb) {
            "POST" -> handlePost()
            "PUT" -> handlePut()
    ...
    private fun handlePostWithForm() {
        ...
        if (requestParams != null) {
            for (key in requestParams!!) {
                params[key!!] = knownParams[key].orEmpty()
            }
        }
        if (basicAuth == null) {
            try { //a few tests trigger the alternative case
                if (getParent()!!.parent != null && getParent()!!.parent  != null) {
    ...
    private fun extractResponseParams(result: RestResult?) {
        ...
        for (assertion in assertions) {
            assertion!!.check(result, getParent()!!.getParent()!!.testId)
        }
        implicitNotFailedAssertion!!.check(result, getParent()!!.getParent()!!.testId)
    }
