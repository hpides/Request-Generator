...
public class Main {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        if(args[0].equals("load") ) {
            ...
            var result = rc.postBodyToEndpoint("REST Test", null, new URL("http://users/users/new"),new ObjectMapper().writeValueAsString(params));
            log.info("Create user: "+result.toString()+" and code: "+result.getReturnCode()+" in: "+result.durationMillis()+" ms.");
            result = rc.getFromEndpointWithAuth("REST Test", null, new URL("http://users/users/all"),null, USERNAME, PASSWORD);
            log.info("Get all users: "+result.toString()+" and code: "+result.getReturnCode()+" in: "+result.durationMillis()+" ms.");
            result = rc.putFormToEndpointWithAuth("REST Test", null, new URL("http://users/users/update"),params, USERNAME, PASSWORD);
            ...
            result = rc.postFormToEndpointWithAuth("REST Test", null, new URL("http://posts/posts/new"),params, USERNAME, PASSWORD);
            log.info("Create post: "+result.toString()+" and code: "+result.getReturnCode()+" in: "+result.durationMillis()+" ms.");
            result = rc.getFromEndpointWithAuth("REST Test", null, new URL("http://posts/posts/all"),null, USERNAME, PASSWORD);
            ...
            result = rc.getFromEndpointWithAuth("REST Test", null, new URL("http://search/posts/search"),params,USERNAME,PASSWORD);
            log.info("Search: "+result.toString()+" and code: "+result.getReturnCode()+" in: "+result.durationMillis()+" ms.");

            log.info("--- Deleting user ---");
            result = rc.deleteFromEndpointWithAuth("REST Test", null, new URL("http://users/users/delete"),null, USERNAME, PASSWORD);
